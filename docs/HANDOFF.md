...
## Admin console, footer sign in, and the 10 MB upload cap

Now that Google sign in works, the admin is an operator account with its own console, and admin sign in moved off the marketing buttons.

Admin sign in moved to the footer. The home page no longer shows an Admin login button next to Continue with Google. Instead there is a quiet Admin sign in link in the page footer, shown only when signed out, that opens the same password modal. Continue with Google is now the single primary call to action.

Admin console. After an admin password login, the app goes to a new route, `/admin`, which renders the admin console instead of the regular forms dashboard. The console is gated to the admin: the session role must be admin, otherwise the route falls back to the normal dashboard. From the console, the logo still goes to the admin's own forms dashboard, and the top bar shows an Admin button to come back. The route name `admin` is reserved, so a public form whose slug is literally `admin` would be shadowed by the console; any other slug is unaffected.

What the console shows. Summary cards across the top: total users, total forms, total responses, open forms, and total storage used out of 10 GB. Below that, a users table: each row is name, email or username, number of forms, number of responses across their forms, storage used, when they joined, and last activity. Click any user to expand their forms, each showing open or closed, title, response count, storage, and slug. It is meant for quick troubleshooting: who is on the system, what they have built, how active they are, and how much they are storing.

Backend. One endpoint powers it, `GET /api/admin/overview`, which returns 403 unless the caller is the admin. It reads all users, all forms, and response counts grouped by form, then computes storage by listing the R2 bucket and summing object sizes per form id (keys are `uploads/<formId>/...`). If R2 is not bound, storage shows as not available and everything else still works. The function is `adminOverview`, with `r2UsageByForm` doing the storage tally.

Ten megabyte upload cap with image auto compression. Uploads are now hard capped at 10 MB. On the public form, when a file is chosen: if it is already under the limit it uploads as is; if it is over and it is an image, the browser compresses it (progressively smaller dimensions and JPEG quality) until it fits, then uploads the smaller version; if it is over and not an image, or an image that still will not fit after compression, the person sees a clear message that it is too big. The server enforces the same 10 MB ceiling as a backstop and returns a too_large error for anything larger, so the cap holds even if the client check is bypassed. The builder's maximum size field is now limited to 10 and the public hint reads up to 10 MB, images shrink to fit. The client helper is `compressImageUnder`; the server cap is the fixed `maxMb` in `uploadFile`.

Verified with a fake database and a stubbed R2 and email: the overview returns correct totals and per user aggregates including storage, a non admin caller gets 403, and a 12 MB upload is rejected with too_large at 10 MB. The console, the signed out footer link, and the form with the new cap and hint all render cleanly.

## Side panel glyphs, e-sign placement and circle field, AI summaries, response selector, Inter font

Six builder fixes in one pass.

Stray characters in the side panel. The builder was showing raw text like the caret escape on every dropdown and on the replace image control. The cause was the known one: unicode escape sequences sitting directly in JSX text nodes render as literal text. The dropdown caret on the themed Select and color picker, and the refresh glyph on the replace image button, now use the actual characters, so they render as the intended small arrow and refresh symbols. If this ever recurs on a new control, the fix is always to use the real character in the markup, never the backslash u form, inside text between tags.

E-sign placement is now a large modal. Placing fields on a document used to happen inside the 320 pixel editor panel, which was too small to be accurate. There is now a Place fields on the document button that opens a wide modal showing the document much larger, with the same pick a field then click to drop interaction, and a small status row listing which fields are placed. This makes positioning far more precise for both image and PDF documents.

New circle fillable field. E-sign fields already supported short text, long text, date, signature, and checkbox. There is now also a circle field. On the document it shows as a faint dashed ring the signer clicks to mark, turning into a solid bold ring, which is useful for circling an option or a spot on a printed form. It is captured like the checkbox and shows as Circled or Not circled in responses. The circle sits transparently over the document rather than in a white box, so it can ring existing printed content.

AI summaries showing only basic. The overview falls back to a basic summary whenever the AI call does not return text. The model string was already current, so the call helper was made more robust: it tries the configured model then known good fallbacks, and it reads the response from several possible fields rather than one. More importantly, the overview card now explains why it is basic when the AI call fails. If the binding is missing it says to add a Workers AI binding named AI to the Pages project and redeploy, which is the most common cause. If the binding exists but errored, it shows the error. Connecting the binding under exactly the name AI and redeploying is what turns basic into generated overviews. The binding name is case sensitive and a redeploy is required after adding it.

Response requirement is now one selector. Every question editor used to show two separate toggles, force response and request response. These are replaced by a single three way selector: Off, Request, Require. Off asks for nothing, Request gently asks but does not block, Require blocks submission until answered. It maps to the same underlying required and request flags, so existing forms keep working unchanged.

Default font is Inter, and the zetetiq font option is gone. The default sans font for forms is now Inter, loaded alongside the existing fonts. The zetetiq option, which used the brand typeface, has been removed from both font pickers. Any older form that had selected it simply falls back to the default sans, which is now Inter.

Verified by build and mount: the compiled output has no literal escape sequences and uses the real glyphs, the three way selector renders in every editor and the old toggles are gone, the e-sign editor shows the placement button and the circle field renders for signers, and the overview note appears with the correct guidance. The robust model handling and the binding diagnostic cannot be exercised without a live Workers AI binding, so confirm richer overviews on the deployed site once the binding named AI is connected and the project is redeployed.

## Toggle geometry, usernames and form links, recessed form frame, carved nav logo, cursor tip flicker, AI thresholds

Toggle switch geometry. The switch track was 38 by 22 with the knob sliding to left 18, which let the knob edge ride past the track on the on state. The track is now 44 by 24 and the knob rests at 3 and slides to 23, so it sits fully inside the track with even margins at both ends. This affects every toggle across all panels.

Usernames and the public link. New Google accounts now get a clean username taken from the part of the email before the at sign, lowercased and stripped to letters, numbers, hyphen and underscore, made unique by appending a number if needed and never a reserved word. This matters because the public form link is built from the username, and an email address as a username carries an at sign and dots that make a fragile URL. People can now also change their username from a new Account button in the top navigation. The rule is more than three characters, only letters numbers hyphen or underscore, and not already used by someone else, checked on the server, which also reissues the session cookie with the new name. The public lookup is now case insensitive as a further safeguard. For an existing test account whose username is still the full email, set a username under Account to get a clean working link.

Recessed form frame. The whole form card, the frame that holds the questions when someone fills out a form, now uses the inset bevel and a transparent border so the entire frame reads as recessed rather than as a raised card.

Carved navigation logo. Once signed in, the zetetiq wordmark in the navigation no longer uses the brand color. It is filled with the surface color and given the same debossed drop shadow as the loading splash, so it reads as carved into the bar with no color.

Cursor tip flicker. The hover tip that follows the cursor was disappearing when the pointer moved from a button onto an icon inside it. The cause was that showing the tip moves the element title into a data attribute to suppress the native tooltip, but the routine that finds the tipped element did not look at that stashed attribute, so a move onto a child made it think there was no tip and hide it. The finder and the text reader now also check the stashed attribute, so the tip stays put across a button and its inner icon.

AI overview thresholds. Overviews now only call the model for accounts that have crossed a usefulness bar: more than five forms, or any single form with more than fifteen responses, for the dashboard overview, and more than fifteen responses for a single form overview. Below that bar the card shows a plain basic summary with no error note, since not generating is intentional rather than a failure. Above the bar it behaves as before, including the binding diagnostic when Workers AI is not reachable.

Verified by build, mount, and backend stubs: the compiled toggle metrics are correct, the Account button opens a modal that validates the username and shows the more than three characters rule, the form frame and nav carving are present, the cursor tip finder reads the stashed title, the username endpoint accepts a valid name and rejects short, reserved, taken, and unauthenticated ones while reissuing the cookie, and the gating returns a basic summary with no error below the thresholds and attempts the model above them.
